...
class Intranet(ABC):
    ...
    @abstractmethod
    async def getActivities(self, year, codeModule, codeInstance):
    ...
    @abstractmethod
    async def getModules(self, year):
...
class Intra(Intranet):
    ...
    def getActivities(self, year, codeModule, codeInstance):
        try:
            data = requests.get(self.token + f"/module/{year}/{codeModule}/{codeInstance}/?format=json")
        except Exception as e:
            print(f'An error occured while asking intra : {e}')
            return
    ...
    def getModules(self, year):
        try:
            data = requests.get(self.token + f"/course/filter?format=json&preload=1&scolaryear[]={year}")
        except Exception as e:
            print(f'An error occured while asking intra : {e}')
            return
